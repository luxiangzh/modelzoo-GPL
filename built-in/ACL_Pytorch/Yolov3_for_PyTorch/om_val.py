...
def main(args):
    ...
    model = aclruntime.InferenceSession(os.path.join(args.output_dir, args.model),
    ...
    for i in tqdm(range(len(dataloader))):
        ...
        img = np.ascontiguousarray(image_np_expanded)

        # om infer
        result = om_infer(model, [img, imginfo])
        batch_boxout = np.array(result[0])
        boxnum = np.array(result[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ground_truth_json', type=str, default="coco2017/instances_val2017.json",
                         help='annotation file path')
    parser.add_argument('--img-path', type=str, default="coco2017/val2017", help='input images dir')
