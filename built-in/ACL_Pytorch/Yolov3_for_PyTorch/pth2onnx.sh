...
if [[ $nms_mode == nms_script ]] ; then
    echo "方式一 nms后处理脚本"
    git apply common/patch/v${tag}.patch --exclude models/yolo.py ## 兼容性修改，支持导出不同版本的YOLOv3
    export=`find ./ -name export.py`  ## 查找export.py文件路径，不同版本路径不一样
    python3 ${export} --weights=${model}.pt --opset=11 --dynamic || exit 1  # 导出onnx模型
    python3 -m onnxsim ${model}.onnx ${model}.onnx || exit 1  # 简化导出的onnx模型
fi

##### 方式二 nms后处理算子
if [[ $nms_mode == nms_op ]] ; then
    echo "方式二 nms后处理算子"
    git apply common/patch/v${tag}.patch  ## 兼容性修改，支持导出不同版本的YOLOv3，Detect部分修改以适配添加nms后处理算子
    export=`find ./ -name export.py`  ## 查找export.py文件路径，不同版本路径不一样
    python3 ${export} --weights=${model}.pt --opset=11 --dynamic || exit 1  # 导出onnx模型
    python3 -m onnxsim ${model}.onnx ${model}.onnx || exit 1  # 简化导出的onnx模型
