...
then
    ...
    taskset -c $p_start-$p_end $CMD python3 -u train.py \
      ...
      --local_rank $i \
      --device_num 8 > ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_full_8p.log 2>&1 &
	done
else
...
python3 val.py \
    ...
    --device npu \
    --batch-size 128 \
    --local_rank 0 > ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_acc_8p.log 2>&1 &

wait

#最后一个迭代FPS值
FPS=`grep -a 'FPS'  ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_full_8p.log|awk 'END {print}'| awk -F "FPS: " '{print $2}'`
