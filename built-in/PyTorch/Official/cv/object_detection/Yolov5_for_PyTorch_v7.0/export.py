...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    if simplify:
        try:
            ...
            check_requirements(('onnxruntime-gpu' if npu_available else 'onnxruntime', 'onnx-simplifier>=0.4.1'))
            import onnxsim
...
def export_saved_model(model,
    ...
    try:
        ...
        check_requirements(f"tensorflow{'' if torch.npu.is_available() else '-macos' if MACOS else '-cpu'}")
        import tensorflow as tf
