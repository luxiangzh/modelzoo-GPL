...
def select_device(device='', npu_id='', batch_size=None):
    # device = 'cpu' or '0' or '0,1,2,3'
    if device.lower() == 'npu':
        if npu_id == -1:
            npu_id = 0
        torch.npu.set_device("npu:%d" % npu_id)
        print('Using NPU %d to train' % npu_id)
        return torch.device("npu:%d" % npu_id)    # device = 'cpu' or '0' or '0,1,2,3'
    ...
    if cuda:
        ...
        if ng > 1 and batch_size:  # check that batch_size is compatible with device_count
            if batch_size % ng != 0:
                raise ValueError('batch-size %g not multiple of GPU count %g' % (batch_size, ng))
