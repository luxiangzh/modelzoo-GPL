...
#训练后进行eval显示精度
python3.7 val.py --data ./data/coco.yaml --conf-thres 0.0005 --iou-thres 0.50 --img-size 640 --weight 'yolov5.pt' --batch-size 128 --device $ASCEND_DEVICE_ID > ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_acc_4p.log 2>&1 &
...
#取acc值
acc=`grep -a 'IoU=0.50     ' ${cur_path}/test/output/$ASCEND_DEVICE_ID/train_acc_4p.log|grep 'Average Precision'|awk 'NR==1'| awk -F " " '{print $13}'`
