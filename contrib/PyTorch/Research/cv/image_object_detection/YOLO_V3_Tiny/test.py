...
def test(cfg,
    ...
    if model is None:
        device = torch_utils.select_device('npu', 0, batch_size=opt.batch_size)
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            inf_out, train_out = model(imgs, augment=augment, test=True)  # inference and training outputs
            t0 += torch_utils.time_synchronized() - t

            # Compute loss
            if hasattr(model, 'hyp') and 0:  # if model has loss hyperparameters
                loss += compute_loss(train_out, targets, model)[1][:3]  # GIoU, obj, cls

            # Run NMS
            t = torch_utils.time_synchronized()
            output = non_max_suppression_yolov5(inf_out, conf_thres=conf_thres, iou_thres=iou_thres)  # nms
                    ...
                    if pi.shape[0]:
                        ...
                        for j in (ious > iouv[0]).nonzero():
                            ...
                            if d not in detected:
                                detected.append(d)
                                correct[pi[j]] = ious[j].npu() > iouv.npu()  # iou_thres is 1xn
    ...
    if save_json and map and len(jdict):
        ...
        with os.fdopen(os.open('results.json', flags, mode), 'w') as file:
