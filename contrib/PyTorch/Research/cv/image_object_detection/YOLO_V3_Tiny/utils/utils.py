...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    ft = torch.npu.FloatTensor if p[0].is_npu else torch.Tensor
    lcls, lbox, lobj = ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec, targets_mask, targets_sum_mask = build_targets(p, targets, model)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        # Compute losses
        nb = len(b)
        if nb or sum_mask.item() > 0:  # number of targets
            ng += nb
            ...
            pwh = torch.exp(ps[2:4, :]).clamp(max=1E3) * anchor_vec[i].T
            ...
            lbox += (1.0 - giou).sum() / sum_mask
            giou = giou * allmask

            confidence = ((1.0 - model.gr) + model.gr * giou.detach().clamp(0)) * allmask
            confidence = confidence.type(tobj.dtype)
            tobj[b, a, gj, gi] = confidence
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                # tmp = ps[5:, :]
                # t = torch.full_like(tmp, cn).to(device)  # targets
                # t[tcls[i], torch.arange(nb, device=device).long()] = cp
                #  t = t * allmask
                # lcls += F.binary_cross_entropy_with_logits(tmp, t,
                #                                        allmask,
                #                                        pos_weight=ft([h['cls_pw']]),
                #                                        reduction=red) * t.size(1) / sum_mask
                #  lcls += binary_cross_entropy(sigmoid(tmp), t, allmask, sum_mask)
...
def build_targets(p, targets, model):
    ...
    na = model.hyp['anchor_number']  # anchors number
    nt, device, batch_size = targets.shape[0], targets.device, p[0].shape[0]
    tcls, tbox, indices, av, targets_mask, targets_sum_mask = [], [], [], [], [], []
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        gain[2:] = torch.tensor(p[i].shape)[[4, 3, 4, 3]]  # xyxy gain
        t = max_target * gain
        allmask = torch.zeros((na * nt_max)).to(device)
        sum_mask = torch.zeros(1).to(device)
        gwh = t[:, 4:6]
        if nt:
            # reject boxes with large proportion
            iou = wh_iou(anchors, gwh)
            t = t.repeat(na, 1, 1).view(-1, 6)  # filter
            ...
            t = t * allmask[:, None]
        # Indices
        b = t.index_select(1, torch.tensor([0], device=device)).long().view(-1)
        c = t.index_select(1, torch.tensor([1], device=device)).long().view(-1)
        gxy = t.index_select(1, torch.tensor([2, 3], device=device))  # target image, class
        gwh = t.index_select(1, torch.tensor([4, 5], device=device))  # grid x, y
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if scores.device.type != 'cuda':
            ...
            screen = torchvision.ops.boxes.nms
        if method == 'merge':  # Merge NMS (boxes merged using weighted mean)
            i = screen(boxes, scores, iou_thres)
...
def binary_cross_entropy(pred, y, allmask, sum_mask):
    # guise_anchor_num = torch.tensor(y.shape[1], dtype=y.dtype, device=y.device)
    nc = torch.tensor(y.shape[0], dtype=y.dtype, device=y.device)
    a = y * pred.log()
    b = (1 - y) * (1 - pred).log() * allmask
    c = -(a + b).sum()
    return c / nc / sum_mask
...
def non_max_suppression_yolov5(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
