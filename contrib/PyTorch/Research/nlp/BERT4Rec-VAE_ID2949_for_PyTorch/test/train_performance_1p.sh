...
cp -r ${data_path}/dataset/Data ${PWD}

sed -i '87s/100/5/' options.py 
# 训练开始时间记录，不需要修改
start_time=$(date +%s)
##########################################################
#########第3行 至 100行，请一定不要、不要、不要修改##########
...
if [ x"${modelarts_flag}" != x ];
then
    python3 main.py --template train_bert  1>${print_log} 2>&1 
else
    python3 main.py --template train_bert --num_epochs=5  1>${print_log} 2>&1 
fi

# 性能相关数据计算
Time=`cat ${print_log} | tr -d '\b\r' | grep -Eo "[0-9]*.[0-9]*it/s" | tail -n +5 | head -n 50 | tail -n +30 | tr -d "it/s" | awk '{sum+=$1} END {print sum/NR}'`
StepTime=`awk 'BEGIN{printf "%.4f\n", '1'/'${Time}'}'`
FPS=`awk 'BEGIN{printf "%.2f\n", '${batch_size}'/'${StepTime}'}'`


# 提取所有loss打印信息
grep -Eo "loss [0-9]*.[0-9]*" ${print_log} | awk '{print $2}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
